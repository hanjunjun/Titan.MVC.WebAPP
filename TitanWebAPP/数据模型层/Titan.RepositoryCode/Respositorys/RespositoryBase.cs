...
namespace Titan.RepositoryCode.Respositorys
{
        ...
        IUnitOfWorkRepository where T : IAggregateRoot, new()
    {
        ...
        /// <param name="predicate"></param>
        /// <returns></returns>
        public int DeleteList(Expression<Func<T, bool>> predicate)
        {
            return context.Set<T>().Where(predicate).Delete();
        }
        ...
        /// <param name="predicate"></param>
        /// <returns></returns>
        public int UpdateList(Expression<Func<T, bool>> predicate, Expression<Func<T, T>> updatepredicate)
        {
            return context.Set<T>().Where(predicate).Update(updatepredicate);
        }
        ...
        {
            return new ModelBaseContext().Set<T>().AsNoTracking().Where(predicate).ToList();
        }
        ...
        {
            context.Set(GetEntityType()).Remove(entity);
            _uow.Commit();
        }
        #endregion
    }
